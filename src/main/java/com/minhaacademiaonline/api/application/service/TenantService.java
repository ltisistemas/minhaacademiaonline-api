...
//        );
    ...
    public String getPublicRegisterStudent(String subdomain) {
        ...
        if (tenant != null) {
            return getUrl(subdomain) + "/api/v1/student/sing-up";
        }
    ...
    @Override
    public byte[] generateQrCodeForStudentSignUp(String subdomain, int width, int height) throws WriterException, IOException {
        Tenant tenant = findTenantBySubdomain(subdomain);
        if (tenant != null) {
            QRCodeWriter qrCodeWriter = new QRCodeWriter();

            String url = getPublicRegisterStudent(subdomain);
